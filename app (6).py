...
with app.app_context():
    try:
        ...
        print("Firebase init warning: " + str(e))
...
def _ts(doc, field='created_at'):
    try:
        v = doc.get(field)
        return v.timestamp() if v and hasattr(v, 'timestamp') else 0.0
    except Exception:
        return 0.0
...
def index():
    ...
    try:
        ...
        trending = sorted(
            [{'id': p.id, **p.to_dict()} for p in db.collection('products').where('active', '==', True).stream()],
            key=lambda x: x.get('sales_count', 0), reverse=True
        )[:8]
...
def marketplace():
    try:
        ...
        if sort == 'price_asc':
            all_prods.sort(key=lambda x: float(x.get('discount_price') or x.get('price', 0)))
        elif sort == 'price_desc':
            all_prods.sort(key=lambda x: float(x.get('discount_price') or x.get('price', 0)), reverse=True)
        elif sort == 'popular':
            all_prods.sort(key=lambda x: x.get('sales_count', 0), reverse=True)
        else:
            all_prods.sort(key=lambda x: _ts(x, 'created_at'), reverse=True)
        if q:
            ql = q.lower()
        ...
        prods, total, total_pages = [], 0, 1
        cat, tag, q, sort, page = '', '', '', 'newest', 1
...
def product(slug):
    try:
        ...
        reviews = sorted(
            [{'id': r.id, **r.to_dict()} for r in db.collection('reviews').where('product_id', '==', p.id).stream()],
            key=lambda x: _ts(x, 'created_at'), reverse=True
        )[:20]
...
def dashboard():
    ...
    try:
        orders = sorted(
            [{'id': o.id, **o.to_dict()} for o in db.collection('orders').where('uid', '==', uid).stream()],
            key=lambda x: _ts(x, 'created_at'), reverse=True
        )[:10]
...
def notifications():
    ...
    try:
        notifs = sorted(
            [{'id': n.id, **n.to_dict()} for n in db.collection('notifications').where('uid', '==', uid).stream()],
            key=lambda x: _ts(x, 'timestamp'), reverse=True
        )[:50]
...
def order_create():
    ...
    if price == 0:
        ...
        notify(uid, 'Order Confirmed', pd.get('name', '') + ' is now available in your orders.', 'success')
...
def order_verify():
    ...
    expected = hmac.new(secret,
                        (d['razorpay_order_id'] + '|' + d['razorpay_payment_id']).encode(),
                        hashlib.sha256).hexdigest()
    if expected != d.get('razorpay_signature'):
        return jsonify({'error': 'Invalid signature'}), 400
    db = get_db()
    ords = list(db.collection('orders')
                .where('razorpay_order_id', '==', d['razorpay_order_id'])
                .where('uid', '==', uid).limit(1).stream())
    if not ords:
    ...
    if pp.exists:
        pp.reference.update({'sales_count': pp.to_dict().get('sales_count', 0) + 1})
    notify(uid, 'Payment Successful',
           'You can now download ' + od.get('product_name', '') + ' from your orders.', 'success')
...
def coupon_check():
    ...
    return jsonify({'valid': True, 'discount': round(disc, 2), 'final': round(final, 2),
                    'msg': 'Coupon applied! You save Rs.' + str(int(disc))})
...
def custom_order():
    if request.method == 'POST':
        ...
        if admin_email:
            body = ('From: ' + str(session.get('name', '')) +
                    ' (' + str(session.get('email', '')) + ')\n' +
                    'Budget: ' + str(request.form.get('budget', '')) + '\n' +
                    'Timeline: ' + str(request.form.get('timeline', '')) + '\n\n' +
                    str(request.form.get('description', '')))
            send_mail_safe(admin_email,
                           'New Custom Order: ' + str(request.form.get('title', '')),
                           body)
        notify(uid, 'Custom Order Received',
               'We received your custom order and will respond within 24 hours.', 'info')
...
def ai_chat_api():
    ...
    try:
        products = []
        for p in get_db().collection('products').where('active', '==', True).limit(40).stream():
            d = p.to_dict()
            products.append({
                'name': d.get('name'), 'category': d.get('category'),
                'price': d.get('price'), 'discount_price': d.get('discount_price'),
                'short_description': d.get('short_description'),
                'slug': d.get('slug'), 'features': d.get('features', [])[:5]
            })
    except Exception:
        products = []
    ...
    try:
        ...
        try:
            get_db().collection('ai_conversations').add({
                'uid': uid, 'message': msg,
...
def admin_dashboard():
    ...
    try:
        ...
        recent = sorted(
            [{'id': o.id, **o.to_dict()} for o in db.collection('orders').stream()],
            key=lambda x: _ts(x, 'created_at'), reverse=True
        )[:8]
...
def admin_products():
    ...
    try:
        prods = sorted(
            [{'id': p.id, **p.to_dict()} for p in get_db().collection('products').stream()],
            key=lambda x: _ts(x, 'created_at'), reverse=True
        )
    except Exception:
        prods = []
...
def admin_product_new():
    ...
    if request.method == 'POST':
        ...
        data = {
            ...
            'category': f.get('category'), 'short_description': f.get('short_description'),
            'description': f.get('description'), 'price': float(f.get('price', 0)),
            'discount_price': float(f.get('discount_price', 0)) or None,
            ...
            'active': 'active' in f, 'featured': 'featured' in f,
...
def admin_orders():
    ...
    try:
        ords = sorted(
            [{'id': o.id, **o.to_dict()} for o in get_db().collection('orders').stream()],
            key=lambda x: _ts(x, 'created_at'), reverse=True
        )
    except Exception:
        ords = []
...
def admin_custom_orders():
    ...
    try:
        ords = sorted(
            [{'id': o.id, **o.to_dict()} for o in get_db().collection('custom_orders').stream()],
            key=lambda x: _ts(x, 'created_at'), reverse=True
        )
    except Exception:
        ords = []
...
def admin_custom_order_status(oid):
    ...
    od = ref.get().to_dict()
    if od:
        notify(od.get('uid', ''), 'Custom Order Update',
               'Your order status: ' + status.replace('_', ' ').title(), 'info')
...
def admin_coupon_new():
    ...
    if f.get('expiry'):
        try:
            expiry = datetime.strptime(f.get('expiry'), '%Y-%m-%d')
        except Exception:
            pass
...
def admin_analytics():
    ...
    try:
        ...
        stats = {'revenue': 0, 'orders': 0, 'products': 0, 'users': 0, 'ai': 0, 'top_products': []}
...
def admin_add_admin():
    ...
    try:
        ...
        flash(email + ' added as admin.', 'success')
    except Exception as e:
...
def admin_backup_create():
    ...
    data = {}
    for c in colls:
        data[c] = [{'id': d.id, **{k: str(v) if hasattr(v, 'timestamp') else v
    ...
    flash('Backup ' + bid + ' created successfully.', 'success')
...
def admin_logs():
    try:
        logs = sorted(
            [{'id': l.id, **l.to_dict()} for l in get_db().collection('admin_logs').stream()],
            key=lambda x: _ts(x, 'timestamp'), reverse=True
        )[:100]
    except Exception:
        logs = []
